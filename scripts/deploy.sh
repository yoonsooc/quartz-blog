...
# 1) Build (v5: 플러그인 설치 → 빌드 — private 섹션은 encrypted-pages가
#    빌드 중 암호화하며, 비밀번호는 section-privacy 플러그인이 env에서 주입)
...
# 1.5) 잔여 유출 표면 정리 (og-image, private 전용 첨부, 파일명 경고)
log "Scrubbing private assets"
node "$ROOT/scripts/scrub-private-assets.mjs"
